...
class ConfigManager {
  ...
  getConfigPath() {
    ...
    // In development mode, use local config directory
    if (process.env.NODE_ENV === 'development' || process.argv.includes('--dev')) {
      return path.join(__dirname, '..', 'config', 'agent-v2.json');
    }
  ...
  getDefaultConfig() {
    return {
      version: '2.0.0',
      agentId: null,
      backendUrl: process.env.BACKEND_URL || 'https://everydaytech.au/api',

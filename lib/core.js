...
class AgentV2 extends EventEmitter {
  ...
  async start() {
    try {
      ...
      this.logger.info('Initializing command executor...');
      this.commandExecutor = new CommandExecutor(this.logger, this.ws);
  ...
  async stop() {
    ...
    // Cleanup command executor (kill active executions)
  ...
  async handleMessage(data) {
    try {
      ...
      switch (message.type) {
        ...
        case 'execute_script':
          if (this.commandExecutor) {
            this.commandExecutor.executeScript(message.execution_id, message.script_id, message.code);
          } else {
            this.logger.warn('CommandExecutor not initialized');
          }
          break;
